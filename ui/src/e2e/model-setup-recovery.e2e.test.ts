...
describeControlUiE2e("Control UI local-provider recovery mocked Gateway E2E", () => {
  ...
  it("shows a failed LM Studio connection with its detected endpoint", async () => {
    ...
    try {
      ...
      await currentConnection.getByRole("button", { name: "Verify connection" }).click();
      await currentConnection.getByText("Unavailable").waitFor();
      await currentConnection.getByText("Needs attention").waitFor();
      await currentConnection.getByText("qwen3-8b-instruct at http://localhost:1234/v1").waitFor();
      await currentConnection
        .getByText("Make sure the provider service is running and reachable, then retry.")
        .waitFor();
      await expect
      ...
      await currentConnection.getByRole("button", { name: "Change connection" }).click();
      ...
      if (artifactDir) {
        await page.screenshot({
          animations: "disabled",
          fullPage: true,
          path: path.join(artifactDir, "local-provider-change-connection-desktop.png"),
        });
      }
    } finally {
      await context.close();
