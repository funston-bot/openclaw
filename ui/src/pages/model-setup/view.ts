...
import {
  failureLabel,
  findPrepareOptionForModel,
  renderCurrentConnection,
  renderModelSetupFailure,
} from "./current-connection.ts";
        ...
        ${candidates.map((candidate) => {
          ...
          const failure =
            ...
            props.activation.targetId === activationTargetId(candidate.kind, candidate.modelRef)
              ? props.activation
              : null;
          const prepareOption = failure
            ? findPrepareOptionForModel(result, candidate.modelRef, candidate)
            : undefined;
          return html`
...
function renderReady(props: ModelSetupViewProps, result: SystemAgentSetupDetectResult) {
  const current = result.configuredModel
    ? renderCurrentConnection({
        ...
        onStartPrepare: props.onStartPrepare,
